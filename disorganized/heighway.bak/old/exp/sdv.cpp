...
typedef struct{ // Vertex structure
	...
	GLfloat tex[2];		// 12 2	// glTexCoordPointer(2, GL_FLOAT, sizeof(Vertex), BUFFER_OFFSET(12));
	GLfloat normal[3];	// 20 3 // glNormalPointer(GL_FLOAT, sizeof(Vertex), BUFFER_OFFSET(20));
	GLfloat colour[4];	// 32 4 // glColorPointer(4, GL_FLOAT, sizeof(Vertex), BUFFER_OFFSET(32));
	GLubyte padding[16];	// 48 4 // up to 64
} Vertex;
